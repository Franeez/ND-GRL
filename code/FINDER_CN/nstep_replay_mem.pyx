...
cdef class py_ReplaySample:
    ...
    cdef G2P(self,Graph graph1): #内部c实现的函数 不给python直接调用
        ...
        return graph.py_Graph(num_nodes,num_edges,cint_edges_from,cint_edges_to,cdouble_edge_value_copy) #返回的是python中的数据类型
...
cdef class py_NStepReplayMem:
    ...
    def Add(self,mvcenv,int nstep):
        ...
        # g = self.GenNetwork(mvcenv.graph)   #networkx？
        g = mvcenv.graph
        ...
        deref(self.inner_Graph).edge_value=g.edge_value
        ...
        deref(self.inner_MvcEnv).prob = mvcenv.prob
    ...
    def Sampling(self,int batch_size):
        ...
        return result #python接口数据

    @property
    def graphs(self):
        result = []
        for graphPtr in deref(self.inner_NStepReplayMem).graphs:
            result.append(self.G2P(deref(graphPtr)))  #edge_value有变化？
        return  result
    @property
    def actions(self):
    ...
    def GenNetwork(self, g):    #networkx2four   没用上
        edges = g.edges() #？？？？
        if len(edges) > 0:
            a, b = zip(*edges)
            A = np.array(a)
