...
SumTree::SumTree(int _capacity){
    capacity = _capacity;
    tree.resize(2 * capacity - 1);//tree的总节点数目
    data.resize(capacity);
    data_pointer = 0; //
    minElement = 2147483647/2; //#2的31次方-1 即有符号最大int/2  ？？
    maxElement = 0.0; //
    //isOverWrite = false;
}
...
void SumTree::Add(double p,std::shared_ptr<Data> _data){
    int tree_idx = data_pointer + capacity - 1; //leaf层从左到右的idx
    data[data_pointer] = _data;
    Update(tree_idx, p);
    data_pointer += 1;
    if (data_pointer >= capacity) //#超过容量从头开始
    {
        //isOverWrite = true;
        data_pointer = 0;
    }
...
std::shared_ptr<LeafResult> SumTree::Get_leaf(double v){
    ...
    while(true){
        ...
        if(cl_idx >= (int)tree.size()){
            leaf_idx = parent_idx;
            break; //#直到从此处推出结束循环 即最后 leaf_idx==parent_idx  叶子和亲戚节点下标相同？？ 不理解
        }
        else{
            if(v <= tree[cl_idx]){
                parent_idx = cl_idx;
...
std::shared_ptr<ReplaySample> Memory::Sampling(int n){ //#n是批量数 每一批多少数据
    ...
    beta = min(1.0, beta + beta_increment_per_sampling);// #beta是大于等于1的值
...
//    double sum=0.0;
    for(int i = 0;i < n;++i){
        double a = pri_seg * i; //#区间左
        double b = pri_seg * (i + 1);//#区间右
        std::default_random_engine random(time(NULL)); //#种子根据time函数 的随机数random
        std::uniform_real_distribution<double> dist(a, b); //#离散均匀分布dist

        double v = dist(random); //#随机取v
...
void Memory::batch_update(std::vector<int> tree_idx, std::vector<double> abs_errors)
{
    ...
    for(int i =0;i<(int)tree_idx.size();++i){
        ...
        tree->Update(tree_idx[i], ps); //#参数对应idx 和 p
    }
}
